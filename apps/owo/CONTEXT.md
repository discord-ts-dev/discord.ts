...
**Toggle**:
A guild-level `enable`/`disable` for one top-level command, offered only for commands declared Toggleable; a group toggles whole, by group name. Enforced by systems `EnabledGuard` through `PlayerGuarded`.
_Avoid_: switch, flag
