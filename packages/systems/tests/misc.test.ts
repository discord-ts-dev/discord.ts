...
describe("splitSubroute", () => {
  test("matches first token case-insensitively", () => {
    expect(splitSubroute(["rr", "2"], ["rr", "lock"])).toEqual({ route: "rr", rest: ["2"] });
    expect(splitSubroute(["LOCK"], ["rr", "lock"])).toEqual({ route: "lock", rest: [] });
  });

  test("no match or no args is null", () => {
    expect(splitSubroute(["x"], ["rr"])).toBeNull();
    expect(splitSubroute([], ["rr"])).toBeNull();
  });
});
