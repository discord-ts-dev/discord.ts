...
async function patch(store: Store, guildId: string, part: Partial<GuildSettings>): Promise<void> {
  await store.set(settingsKey(guildId), JSON.stringify({ ...(await getSettings(store, guildId)), ...part }));
}
...
export async function setCommandEnabled(store: Store, guildId: string, command: string, enabled: boolean): Promise<void> {
  const name = command.toLowerCase();
  const disabled = new Set(((await getSettings(store, guildId)).disabled ?? []).map((c) => c.toLowerCase()));
  if (enabled) disabled.delete(name);
  else disabled.add(name);
  await patch(store, guildId, { disabled: [...disabled] });
}

export async function isCommandEnabled(store: Store, guildId: string, command: string): Promise<boolean> {
